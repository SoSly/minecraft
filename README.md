## Server Rules
1. This is Niv's personal world.
    1. Suggestions and bug reports are welcome
    2. Complaints about things you don't like are not welcome
    3. Ask before completing endgame content (such as bosses) without Niv
2. Avoid lagging the server where possible
    1. Don't spam the server with entities
    2. Don't run laggy redstone contraptions
    3. Don't build near 0 ~ 0 without permission.
3. Respect player properties
    1. If you plan to build within 128 chunks of another player, make sure your build does not conflict with their aesthetic, first.
    2. Fix anything you break
    3. Don't take resources without asking
    4. If you spot a disabled mob spawner, don't destroy it
4. Don't destroy the world within 500 chunks of 0, 0
    1. Don't summon the Wither in the Overworld.
    2. Don't destroy or strip villages or other surface structures within that range
    3. Don't leave creeper holes lying around within that range
    4. Don't terraform the surface within that range without discussing it with Niv first.
5. Don't engage in intentional PvP Combat
    1. Contained competitive games (such as spleef) where everyone involved has explicitly agreed to the PvP are fine
6. If something seems easily abused or too good to be true, report it.  Don't abuse it.
7. Do not share the world details for this server outside of the #minecraft channel on SoSly's Discord.
8. Do not travel to other dimensions until Niv has opened them for play. Currently open dimensions:
    - the overworld

## Planned Mods (in order of priority)
### Graphics and UI
- Controlling
- AppleSkin
- Overloaded Armor Bar
- Ding
- Polymorph
- Better Advancements
- Blur
- Embeddium
- TexTrue's Embeddium Options
- Oculus
- Farsight (and/or Distant Horizons)
- Mouse Tweaks
- JourneyMap
- Jade
- Just Enough Items
- Embeddium/Rubidium Dynamic Lights
- Radiant Gear
- Traveler's Titles
- Presence Footsteps
- Sound Physics Remastered
- BlueMap
- Day Counter
- FPS Monitor
- Yeetus Experimentus
- Better Animations Collection
- Cosmetic Armor Reworked

### Core
- Simple Backups
- Mana & Artifice
    - Arcane Additions
- MineColonies
- Farmer's Delight
- Tetra
- Croptopia
- Better Combat
    - Tetratic Combat Expanded
- Create
- End Remastered
- Quark
- Integrated Stronghold
- Rats
- Weather, Storms & Tornadoes
- KubeJS
    - KubeJS Create

### World Generation
- Continents*
- Tectonic*
- Biome Replacer*
- William Wyther's Overhauled Overworld*
- Simply Improved Terrain*
- Deeper Oceans
- Ecologics
- Biomes O' Plenty
- Tax' Village Architect

\* These mods may be replaced by Reterraforged

### Server Performance
- In Control!
- Dynamic View
- Connectivity
- AttributeFix
- FerriteCore
- Too Fast
- Fast Leaf Decay
- Entity Culling
- Let Me Despawn
- Smooth Chunk Save
- Chunk Pregenerator
- Clumps
- MC2Discord

### Adventure
- Aquaculture 2
- Aquamirae
- Yung's Better Ocean Monuments
- YUNG's Better Desert Temples
- YUNG's Better Jungle Temples
- YUNG's Better Witch Huts
- Hopo Better Mineshaft (or YUNG's Better Mineshafts)
- Hopo Better Ruined Portals
- Hopo Better Underwater Ruins
- Ice & Fire Dragons
- Naturalist

### Gameplay
- MmmMmmMmmMmm (Target Dummy)
- GraveStone Mod
- PlayerRevive
- Klee Slabs
- Better Safe Bed
- Ping Wheel
- [Let's Do] Bakery
- [Let's Do] Meadow
- [Let's Do] BloomingNature
- [Let's Do] Herbal Brews
- [Let's Do] Vinery
- [Let's Do] Candlelight
- [Let's Do] Brewery
- Comforts
- Chime
- Fairy Lights
- Croptopia Delight
- Fruits Delight
- Aquaculture Delight
- Ocean's Delight
- Crabber's Delight
- Chef's Delight
- Quark Delight
- Maturi Delight
- Better Farmer's Combat
- Better Days
- Tetraonimicon
- Tetra's Delight
- Small Ships
- Passable Foliage
- AstikorCarts Redux
- Better Falling
- Aqua Combat
- Charm of Undying
- Scaffolding Drops Nearby
- Craft Bundle TFC
- Craftable Chainmail Armor
- Craftable Horse Armour & Saddle
- Create Recycle Everything
- Create Bells & Whistles
- Create Steam 'n' Rails
- Enchanted Book Redesign
- Enchantment Descriptions
- Floral Enchantment
- Fabulous Fletching
- Simply Steel

### Decoration
- Supplementaries
- Supplementaries Squared
- Beautify!
- Dawn of Time - Builder Edition
- Tanuki Decor
- Stoneworks
- Grass Slabs, Carpets & Stairs
- Create Deco
- Handcrafted
- Tetra tables looking neat
- Amendments
- Aquamirae Extra Music Mod
- Additional Banners
- Bedspreads
- Chipped
- Create Design n' Decor
- Create Interiors
- Decorative Storage
- Immersive Paintings
- Path Under Gates
- Perfect Plushies
- Pretty Beaches
- Storage Decorations

### The Nether
- NetherPortalFix
- YUNG's Better Nether Fortress
- [Let's Do] Nether Vinery
- Progressive Bosses
- Bad Wither No Cookie - Reloaded
- Nether's Delight

### The End
- YUNG's Better End Island
- Elytra Slot
- End's Delight
- Ender's Delight

### For Later Consideration
- Almost Unified
- JadeColonies
- Serene Seasons
- Snow! Real Magic!
- Unusual Delight
- WorldEdit
- YUNG's Extras
- EveryCompat
- Universal Sawmill
- Immersive Weathering
- Vertical Slabs Compat
- V-Tweaks